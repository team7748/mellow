...
it("filters the registry list by tense group", async () => { window.location.hash = "#speak?view=grammar"; render(<SpeakModePage />); await userEvent.setup().click(await screen.findByRole("button", { name: "past" })); expect(screen.getByText("Past Simple")).toBeInTheDocument(); expect(screen.queryByText("Future Simple")).not.toBeInTheDocument() })
